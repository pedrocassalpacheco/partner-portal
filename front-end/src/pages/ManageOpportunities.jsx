...
import { useOpportunities, useOpportunityMutations } from '../hooks/useOpportunity'
import { VALID_STATUSES, STATUS_CONFIG } from '../config/statuses'

function ManageOpportunities() {
  const navigate = useNavigate()
  
  // Use custom hooks for opportunity data and operations
  const {
    opportunities,
    loading,
    error,
  ...
  const { deleteOpportunity, updateStatus } = useOpportunityMutations()
  ...
  const handleStatusClick = (opportunity, status) => {
  ...
  const handleStatusChange = async () => {
    try {
      await updateStatus(selectedOpportunity._key, {
        newStatus,
      ...
      alert('Error updating opportunity status: ' + err.message)
    }
  }
  ...
  const handlePause = (opportunityId) => {
    console.log('Pausing opportunity:', opportunityId)
  ...
  return (
    <Layout>
      <div style={{ marginBottom: 'var(--space-8)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ marginBottom: 'var(--space-4)' }}>Manage Opportunities</h1>
          <p style={{ fontSize: 'var(--text-base)', color: 'var(--gray-700)' }}>
            {pagination.total > 0 && `Showing ${((pagination.page - 1) * pagination.limit) + 1}-${Math.min(pagination.page * pagination.limit, pagination.total)} of ${pagination.total} opportunities`}
            {pagination.total === 0 && 'View and manage all opportunities.'}
          </p>
        </div>
        <button
      ...
      <div style={{
        ...
        <div>
          ...
          <input
            ...
            onChange={handleFilterChange('partnerId')}
            style={{
              width: '100%',
              padding: 'var(--space-3)',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          <button
            onClick={() => {
              updateFilters({ partnerId: '', status: '' })
            }}
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: 'var(--space-12)' }}>
          <p style={{ color: 'var(--gray-600)' }}>Loading opportunities...</p>
        </div>
      )}
      ...
      {!loading && !error && opportunities.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 'var(--space-12)' }}>
          <h3 style={{ color: 'var(--gray-600)', marginBottom: 'var(--space-4)' }}>No Opportunities Found</h3>
          <p style={{ color: 'var(--gray-500)' }}>
            {filters.partnerId || filters.status 
              ? 'No opportunities match your filters. Try adjusting your search criteria.'
              : 'There are no registered opportunities yet.'}
          </p>
        </div>
      )}
                ...
                {opportunities.map((opportunity, index) => (
                  ...
                  >
                    ...
                    <td style={{ 
                      ...
                      <button
                        onClick={() => navigate(`/opportunities/${opportunity._key}`)}
                        style={{
                          background: 'none',
                          border: 'none',
                    ...
                    <td style={{ 
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-1)' }}>
                        {opportunity.productSkus?.slice(0, 2).map((sku, i) => (
                          <span key={i} style={{
                            fontSize: 'var(--text-xs)',
                            padding: '2px 6px',
                        ...
                        {opportunity.productSkus?.length > 2 && (
                          <span style={{
                            fontSize: 'var(--text-xs)',
                            padding: '2px 6px',
                            color: 'var(--gray-500)',
                    ...
                    <td style={{ 
                      ...
                      {formatCurrency(opportunity.budgetaryAmount)}
                    </td>
                    <td style={{ padding: 'var(--space-4)' }}>
                      <span style={{
                        ...
                        fontSize: 'var(--text-xs)',
                        fontWeight: 'var(--font-weight-medium)',
                        backgroundColor: STATUS_CONFIG[opportunity.status]?.backgroundColor || 'var(--gray-100)',
                        color: STATUS_CONFIG[opportunity.status]?.color || 'var(--gray-700)',
                        border: `1px solid ${STATUS_CONFIG[opportunity.status]?.borderColor || 'var(--gray-300)'}`
                      }}>
                      ...
                      <div style={{ display: 'flex', gap: 'var(--space-2)', justifyContent: 'flex-end' }}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                          <button
                            ...
                            title={`Change Status (Current: ${STATUS_CONFIG[opportunity.status]?.label || 'N/A'})`}
                            style={{
                              padding: '6px 12px',
                              fontSize: 'var(--text-sm)',
                              backgroundColor: STATUS_CONFIG[opportunity.status]?.backgroundColor || '#d1ecf1',
                              border: `1px solid ${STATUS_CONFIG[opportunity.status]?.borderColor || '#17a2b8'}`,
                              borderRadius: 'var(--radius-sm)',
                              color: STATUS_CONFIG[opportunity.status]?.color || '#0c5460',
                              cursor: 'pointer',
                          ...
                          >
                            <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
                              {STATUS_CONFIG[opportunity.status]?.icon === 'check' && (
                                <path d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z"/>
                              )}
                              {STATUS_CONFIG[opportunity.status]?.icon === 'x' && (
                                <path d="M3.72 3.72a.75.75 0 011.06 0L8 6.94l3.22-3.22a.75.75 0 111.06 1.06L9.06 8l3.22 3.22a.75.75 0 11-1.06 1.06L8 9.06l-3.22 3.22a.75.75 0 01-1.06-1.06L6.94 8 3.72 4.78a.75.75 0 010-1.06z"/>
                              )}
                              {(STATUS_CONFIG[opportunity.status]?.icon === 'alert' || !opportunity.status) && (
                                <path d="M8 2a6 6 0 100 12A6 6 0 008 2zM7 7V5h2v2H7zm0 4v-2h2v2H7z"/>
                              )}
                            </svg>
                            <span style={{ fontSize: 'var(--text-xs)', textTransform: 'capitalize' }}>
                              {STATUS_CONFIG[opportunity.status]?.label || opportunity.status || 'N/A'}
                            </span>
                            ...
                            {VALID_STATUSES.map((status, index) => {
                              ...
                              return (
                                <button
                                  key={status}
                                  onClick={() => {
                                    handleStatusClick(opportunity, status)
                        ...
                        <button
                          onClick={() => handleEdit(opportunity._key)}
                          title="Edit"
                          style={{
                            padding: '6px 10px',
                        ...
                        <button
                          onClick={() => handlePause(opportunity._key)}
                          title="Pause"
                          style={{
                            padding: '6px 10px',
                            fontSize: 'var(--text-base)',
                        ...
                        <button
                          onClick={() => handleDelete(opportunity._key)}
                          title="Delete"
                          style={{
                            padding: '6px 10px',
                            fontSize: 'var(--text-base)',
      ...
      {showStatusDialog && selectedOpportunity && (
        <div
          style={{
            position: 'fixed',
            top: 0,
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h3 style={{ marginTop: 0, marginBottom: 'var(--space-4)', fontSize: 'var(--text-xl)', fontWeight: 'var(--font-weight-semibold)' }}>
              Confirm Status Change
            </h3>
            <p style={{ marginBottom: 'var(--space-4)', color: 'var(--gray-700)', fontSize: 'var(--text-sm)' }}>
              <strong>{selectedOpportunity.accountName}</strong>
            </p>
            <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--gray-700)' }}>
                  Current Status
                </label>
                <div style={{
                  padding: 'var(--space-2) var(--space-3)',
                  backgroundColor: STATUS_CONFIG[selectedOpportunity.status]?.backgroundColor || '#d1ecf1',
                  color: STATUS_CONFIG[selectedOpportunity.status]?.color || '#0c5460',
                  border: `1px solid ${STATUS_CONFIG[selectedOpportunity.status]?.borderColor || '#17a2b8'}`,
                  borderRadius: 'var(--radius-md)',
                  fontSize: 'var(--text-sm)',
                  fontWeight: 'var(--font-weight-medium)',
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{
                  ...
                  border: `1px solid ${STATUS_CONFIG[newStatus]?.borderColor || '#17a2b8'}`,
                  borderRadius: 'var(--radius-md)',
                  fontSize: 'var(--text-sm)',
                  fontWeight: 'var(--font-weight-medium)',
                  textTransform: 'capitalize'
            ...
            <div style={{ marginBottom: 'var(--space-4)' }}>
              <label htmlFor="statusComment" style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--gray-700)' }}>
                Comment (optional)
              </label>
              <textarea
                id="statusComment"
